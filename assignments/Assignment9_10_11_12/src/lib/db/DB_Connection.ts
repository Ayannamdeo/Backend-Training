...
class DB_Connection {
  ...
  private constructor(url: string) {
    this.mongoUrl = url;
    logger.debug('DB_Connection constructor called', {mongoUrl: this.mongoUrl});
  }
  ...
  public static getInstance(mongoUrl: string): DB_Connection {
    if (!DB_Connection.instance) {
      ...
      logger.info("new db_connection instance created", {instance: DB_Connection.instance});
    } else {
      logger.debug("db_connection instance reused", {instance: DB_Connection.instance});
    }
