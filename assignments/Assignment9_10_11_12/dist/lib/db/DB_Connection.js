...
class DB_Connection {
    constructor(url) {
        this.mongoUrl = url;
        logger_1.logger.debug('DB_Connection constructor called', { mongoUrl: this.mongoUrl });
    }
    static getInstance(mongoUrl) {
        if (!DB_Connection.instance) {
            DB_Connection.instance = new DB_Connection(mongoUrl + "CRUD");
            logger_1.logger.info("new db_connection instance created", { instance: DB_Connection.instance });
        }
        else {
            logger_1.logger.debug("db_connection instance reused", { instance: DB_Connection.instance });
        }
